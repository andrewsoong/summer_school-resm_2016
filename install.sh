...
cd src
./configure --prefix=${PROGS}/netcdf-${NCCC_VER} --disable-dap CC=${CC} FC=${FC} LDFLAGS="-L${PROGS}/zlib-${ZLIB_VER}/lib -L${PROGS}/hdf5-${HDF5_VER}/lib" CPPFLAGS="-I${PROGS}/zlib-${ZLIB_VER}/include -I${PROGS}/hdf5-${HDF5_VER}/include"
make > make.log
make install >> make.log
...
# put all environment variable definition to a file
...
if [ "$FC" == "gfortran" ]; then
  ...
  echo "export ESMF_COMM=intelmpi" >> env_progs
fi
